...
const OpeningForm1: React.FC = () => {
  ...
  const [closingDip, setClosingDip] = useState<number | null>(null);
  const [closingSonding, setClosingSonding] = useState<number | null>(null);
  const [flowMeterEnd, setFlowMeterEnd] = useState<number | null>(null);
  const [hmEnd, setHmEnd] = useState<number | null>(null);
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchLastLkfData = async (station: string) => {
      try {
        ...
        if (data && data.status === "200") {
          ...
          setClosingSonding(closing_dip); // If you need this state variable
          setFlowMeterAwal(flow_meter_end); // Set Flow Meter Awal from fetched data
          setHmAwal(hm_end); // Set HM Awal from fetched data
          setOpeningSonding(closing_sonding); // Set Opening Sonding from fetched data
          setOpeningDip(closing_dip); // Assuming you want to set Opening Dip similarly
        } else {
    ...
    if (loginData) {
      ...
      setStation(stationFromLogin);
      fetchLastLkfData(stationFromLogin);
    }
  }, []);
  ...
  const fetchLastLkfData = async (station: string) => {
    try {
      ...
      if (data && data.status === "200") {
        console.log(`Fetched data for ${station}:`, data.data); // Log the complete data response
        const { closing_dip, closing_sonding, flow_meter_end, hm_end, close_data, opening_dip, site, shift, date } = data.data;
  
        // Update state with the fetched values
        setClosingDip(closing_dip);
        setClosingSonding(closing_sonding);
        setFlowMeterEnd(flow_meter_end);
        setHmEnd(hm_end);
    
        setOpeningDip(opening_dip);
        setSite(site);
      
        setDate(date);
      } else {
        console.error('Unexpected data format or status:', data);
      }
    } catch (error) {
      console.error('Error fetching last LKF data:', error);
    }
  ...
  return (
    <IonPage>
      ...
      {station && (
        <div>
          <p>Station: {station}</p>
          <p>Closing Dip: {closingDip !== null ? closingDip : 'Loading...'}</p>
          <p>Closing Sonding: {closingSonding !== null ? closingSonding : 'Loading...'}</p>
          <p>Flow Meter End: {flowMeterEnd !== null ? flowMeterEnd : 'Loading...'}</p>
          <p>HM End: {hmEnd !== null ? hmEnd : 'Loading...'}</p>
        </div>
      )} */}
    </div>
        ...
        <IonItem key={shift.id} className="item-no-border">
          ...
          <div className="padding-content">
            ...
            <IonInput
              ...
              type="number"
              value={openingDip}
              onIonInput={(e) => setClosingSonding(Number(e.detail.value))}
            />
