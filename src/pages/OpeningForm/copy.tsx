...
const OpeningForm: React.FC = () => {
  const [closingdip, setOpeningDip] = useState<number | undefined>(undefined);
 ...
 const [closing, setClosingDip] = useState<number | undefined>(undefined);
...
>([]);
  ...
  //   debounce(async (closingDip: number | undefined, station: string | undefined) => {
  //     if (closingDip !== undefined && station !== undefined) {
  //       try {
  //         if (closingDip === 0 && station === 'loginData') {
  //           setClosingSonding(0);
  //         } else {
  //           const matchingData = sondingMasterData.find(
  //             (item) => item.station === station && item.cm === closingDip
  //           );
  //           if (matchingData) {
  //             setClosingSonding(matchingData.liters);
  //           } else {
  //             setClosingSonding(undefined);
  //           }
  //         }
  //       } catch (error) {
  ...
  // useEffect(() => {
  //   debouncedUpdate(closingSonding, station);
  // }, [closingSonding, station, debouncedUpdate]);
...
useEffect(() => {
  ...
  const initialFetchTimeout = setTimeout(() => {
    fetchData(); // Initial data fetch after 3 seconds
  }, 3000);
    ...
    <IonPage>
      ...
      <IonRefresher slot="fixed" onIonRefresh={doRefresh}>
        ...
        <div className="wrapper-content">
          ...
          <div className="padding-content">
            <IonLabel className={showError && (closingdip === undefined || Number.isNaN(closingdip) || closingdip < 100) ? "error" : ""}>
              Opening Dip (Liter) <span style={{ color: "red" }}>*</span>
            </IonLabel>
            <IonInput style={{ background: "#cfcfcf" }}
              className={`custom-input ${showError && (closingdip === undefined || Number.isNaN(closingdip) || closingdip< 100) ? "input-error" : ""}`}
              type="number"
              placeholder="Input opening dip dalam liter"
              value={closingdip}
              // readonly={stationOptions.includes(station ||'')}
              onIonInput={(e) => setClosingDip(Number(e.detail.value))}
            />
            {showError && closingdip === undefined && (
              <p style={{ color: "red" }}>* Field harus diisi</p>
            )}
          </div>
          <div className="padding-content">
