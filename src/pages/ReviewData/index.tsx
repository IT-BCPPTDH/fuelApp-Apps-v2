...
const ReviewData: React.FC = () => {
  ...
  const [closeSonding, setCloseSonding] = useState<number | undefined>(
    undefined
  );
  ...
  const [flowMeterStart, setFlowMeterStart] = useState<number | undefined>(
    undefined
  );
    ...
    const getcardDash = () => {
      try {
        ...
        if (cachedData) {
          ...
          const totalIsssued = cardDash.find(
            (item: { title: string }) => item.title === "QTY Issued"
          );
          const closeData = cardDash.find(
            (item: { title: string }) => item.title === "Stock On Hand"
          );
          const flowMeterAwal = cardDash.find(
            (item: { title: string }) => item.title === "Flow Meter Awal"
          );
          const totalMeter = cardDash.find(
            (item: { title: string }) => item.title === "Flow Meter Akhir"
          );
          const totalReceipt = cardDash.find(
            (item: { title: string }) => item.title === "Receipt"
          );
          const openingDip = cardDash.find(
    ...
    <IonPage>
      ...
      <IonContent>
        <div style={{ marginTop: "18px", padding: "18px" }}>
          <IonList>
            <IonListHeader>
              <IonLabel style={{ fontSize: "24px" }} className="font-review">
                Review Data
              </IonLabel>
            </IonListHeader>
            <IonItem>
              <IonLabel style={{ fontSize: "18px" }} className="font-review">
                Open Sonding / Dip:{" "}
                {openingSonding !== undefined ? openingSonding : "Loading..."}{" "}
                Cm / {openingDip} Liters
              </IonLabel>
            </IonItem>
            <IonItem>
              <IonLabel style={{ fontSize: "18px" }} className="font-review">
            ...
            <IonItem>
              <IonLabel style={{ fontSize: "18px" }} className="font-review">
                Closing Sonding / Dip : {closeSonding} Cm / {closingDip} Liter{" "}
              </IonLabel>
            </IonItem>
            <IonItem>
              <IonLabel style={{ fontSize: "18px" }} className="font-review">
                Flow Meter Akhir: {flowMeteAkhir}
              </IonLabel>
            </IonItem>
            <IonItem>
              <IonLabel style={{ fontSize: "18px" }} className="font-review">
          ...
          <div style={{ marginTop: "18px", float: "inline-end" }}>
            <IonButton
              color="light"
              onClick={() => route.push("/closing-data")}>
