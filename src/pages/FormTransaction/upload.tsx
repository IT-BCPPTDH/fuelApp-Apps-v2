...
const handleQuantityChange = (e: any) => {
    ...
    if (typeof selectedUnit === 'string' && (selectedUnit.startsWith("LV") || selectedUnit.startsWith("HLV"))) {
        ...
        if (isNaN(inputQuantity) || isNaN(remainingQuota)) {
            console.error("Invalid numbers detected:", inputQuantity, remainingQuota);
            return;
          }
...
//     if (typeof selectedUnit === 'string' && (selectedUnit.startsWith("LV") || selectedUnit.startsWith("HLV"))) {
//       if (typeof remainingQuota !== 'number' || isNaN(remainingQuota)) {
//         setQuantityError("Sisa kouta tidak valid.");
//         setIsError(true);
//         return; // Keluar jika remainingQuota tidak valid
//       }
            ...
            />
              ...
              <IonRow>
                    <IonCol>
                        ...
                        {/* Display error if the quantity is undefined */}
                        {showError && quantity === undefined && (
                        <p style={{ color: "red" }}>* Field harus diisi</p>
                        )}

                        {/* Display error if quantity is greater than remainingQuota */}
                        {typeof selectedUnit === 'string' && (selectedUnit.startsWith("LV") || selectedUnit.startsWith("HLV")) && (
                        quantity !== null && quantity > remainingQuota && (
                            <p style={{ color: "red" }}>
                            Qty Issued tidak boleh lebih besar dari sisa kouta. Mohon hubungi admin agar bisa mengisi kembali !!
                            </p>
                        )
                        )}
                    </IonCol>
            ...
            />
              ...
              <div style={{ marginTop: "60px", float: "inline-end" }}>
                ...
                <IonButton
                    ...
                    className={`check-button ${isOnline ? "button-save-data" : "button-save-draft"}`}
                    disabled={
                        quantity === null || // Jika quantity null
                        quantity > remainingQuota || // Jika quantity lebih besar dari remainingQuota
                        !quantity // Jika quantity belum diisi
                      }
                    >
                    <IonIcon slot="start" icon={saveOutline} />
                    {isOnline ? "Simpan Data" : "Simpan Data Ke Draft"}
